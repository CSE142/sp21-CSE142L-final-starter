			...
			for (int offset_y = 0; offset_y < MAX_OFFSET; offset_y++)  {
				...
				for(int pixel_x = 0; pixel_x < images.size.x; pixel_x++) {
					...
					for(int pixel_y = 0; pixel_y < images.size.y; pixel_y++) {
						...
						// Note that x is the first argument.  Incrementing x moves to the next item in the tensor's
						// internal arrray.
	...
	for (int this_frame = 1; this_frame < images.size.b; this_frame++) {
		...
		for (int offset_x = 0; offset_x < MAX_OFFSET; offset_x++)  {
			for (int offset_y = 0; offset_y < MAX_OFFSET; offset_y++)  {
				...
				for(int pixel_y = 0; pixel_y < images.size.y; pixel_y++) {
					// Put x in the innermost loop, so the inner loop
					// iterates over consecutive elements in the tensor.
					for(int pixel_x = 0; pixel_x < images.size.x; pixel_x++) { 
...
void do_stabilize_pretile_y(const tensor_t<double> & images, tensor_t<double> & output, int TILE_SIZE)
{
	...
	for (int this_frame = 1; this_frame < images.size.b; this_frame++) {
		...
		for (int offset_x = 0; offset_x < MAX_OFFSET; offset_x++)  {
			for (int offset_y = 0; offset_y < MAX_OFFSET; offset_y++)  {
				// Tile the y loop.  So far, this changes nothing.
...
void do_stabilize_iteration_space(const tensor_t<double> & images, tensor_t<double> & output, int TILE_SIZE)
{
	const int F = 1;//TILE_SIZE;
	const int X = TILE_SIZE;
	const int Y = TILE_SIZE;

	for(int ff = 1; ff < images.size.b; ff += F) {
		for(int pixel_yy = 0; pixel_yy < images.size.y; pixel_yy += Y) {
			for(int pixel_xx = 0; pixel_xx < images.size.x; pixel_xx += X) {

				for(int f = ff; f < ff + F && f < images.size.b; f++) {
					int previous_frame = f - 1;
					
					for(int pixel_y = pixel_yy; pixel_y < pixel_yy + Y && pixel_y < images.size.y; pixel_y++) {
						for(int pixel_x = pixel_xx; pixel_x < pixel_xx + X && pixel_x < images.size.x; pixel_x++) {
							...
							for (int offset_y = 0; offset_y < MAX_OFFSET; offset_y++)  {
								for (int offset_x = 0; offset_x < MAX_OFFSET; offset_x++)  {
									...
									output(offset_x, offset_y, 0, f) += 
										fabs(images(pixel_x, pixel_y, 0, f) -
										     images(shifted_x, shifted_y, 0, previous_frame));
								}
							}
						}
...
void do_stabilize_offset_inside(const tensor_t<double> & images, tensor_t<double> & output, int TILE_SIZE)
{
	const int F = 1;//TILE_SIZE;
	const int X = TILE_SIZE;
	const int Y = TILE_SIZE;
...
void do_stabilize_offset_outside(const tensor_t<double> & images, tensor_t<double> & output, int TILE_SIZE)
{
	const int F = 1;//TILE_SIZE;
	const int X = TILE_SIZE;
	const int Y = TILE_SIZE;
	...
	for(auto &i: impl_map) {
		if (version == i.first || version == "all") {
			...
			name << i.first << "_" << tile_size;
			{
				...
				timer.attr("function", i.first)
					.attr("tile_size", tile_size)
					.attr("frames", frames)
					.go();
				NEW_TRACE(name.str().c_str());
